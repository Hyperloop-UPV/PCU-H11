...
public:
    ...
    static void Speed_data_init(float &target_speed, double &speed_error, float &actual_current_ref, float &slip_motor, uint32_t &cnt_encoder, double &speed_km_h_IMU)
    {
        Speed_data_packet = new HeapPacket(static_cast<uint16_t>(555), &target_speed, &speed_error, &actual_current_ref, &slip_motor, &cnt_encoder, &speed_km_h_IMU);
    }
