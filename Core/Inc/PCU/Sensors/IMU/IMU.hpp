...
class IMU{
	...
	static inline constexpr double accel_sensitivity = 16384;
	static inline constexpr double gyro_sensitivity = 16.4;
	...
	ST_LIB::SPIDomain::Instance* spi_pins;
	...
	static void SPI_transmit(const span<uint8_t> data) {
	LV_BMS::spi_wrapper->send(data);
	}

	static void SPI_receive(span<uint8_t> buffer) {
